"""
mortality.py — Vectorized Scorch Height and Mortality (V7)
...
def build_mortality_lookup(species_master_table: Optional[object] = None) -> np.ndarray:
    ...
    for eq, b in _CROWN_SCORCH_COEFFS_BY_EQ.items():
        if 0 < eq <= _MAX_EQ:
            coeffs[eq] = b
    # Populate bole-char equations (100–109)
...
def calculate_scorch_height(
        fireline_intensity_btu_ft_s: Union[float, np.ndarray],
        midflame_wind_mph: Union[float, np.ndarray],
        air_temp_f: Union[float, np.ndarray]
) -> np.ndarray:
    """
    Vectorized scorch height (Van Wagner 1973 / Rothermel 1976).

    :param fireline_intensity_btu_ft_s: Fireline intensity (*S) or scalar (BTU/ft/s).
    :param midflame_wind_mph: Midflame wind speed (*S) or scalar (mph).
    :param air_temp_f: Air temperature (*S) or scalar (°F).
    :return: (*S) ndarray — scorch height (ft). Returns 0.0 where
    ...
    # Temperature differential between lethal scorch threshold (140°F) and air
    denom     = 140.0 - t
...
def calculate_crown_scorch_mortality(
    ...
    """
    Vectorized crown scorch mortality (Ryan & Reinhardt 1988 logistic model).

    :param scorch_height_ft: Scorch height (ft) (*S), from
        ``calculate_scorch_height()``.
    :param tree_height_ft: Tree height (ft) (*S).
    :param crown_ratio: Live crown ratio as fraction (0–1) (*S).
    :param dbh_inches: Diameter at breast height (inches) (*S).
    :param equation_number_grid: Per-cell mortality equation number (*S) int.
        Crown-scorch equations are 1–20; bole-char equations are 100–109.
    :param coeffs: B-coefficient array of shape (``_MAX_EQ`` + 1, 3) from
        ``build_mortality_lookup()``.
    :return: dict with keys:
        ``'crown_length_scorch'`` (*S) — fraction of crown length scorched (0–1);
        ``'crown_volume_scorch'`` (*S) — fraction of crown volume scorched (0–1);
    ...
    # Crown length scorch — fraction of crown length that is scorched
    cls = np.where(
        crown_length > 1e-7,
    ...
    # Logistic mortality probability: P = 1 / (1 + exp(-(B1 + B2*dbh + B3*cvs)))
    logit = B1 + B2 * dbh + B3 * cvs
    prob  = 1.0 / (1.0 + np.exp(-logit))

    # Zero out cells with no equation defined (B1=B2=B3=0 would yield spurious 0.5)
