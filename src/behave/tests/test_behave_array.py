"""
test_behave_array.py — Test suite for the vectorized (*_array.py) modules.

Structure mirrors test_behave.py:
  - Same TestInfo / report_test_result harness
  - Same input scenarios and expected values (derived from scalar path)
  - Additional array-specific tests: shape preservation, heterogeneous
    fuel grids, no-data masking, multi-cell batch correctness
  - Every numerical expected value is obtained from the scalar BehaveRun
    path so array ↔ scalar parity is the primary assertion.

Run from src/behave/:
    python tests/test_behave_array.py
"""
...
from behave_array import BehaveRunArray
...
def test_surface_unit_conversion(ti):
    ...
    from components.behave_units_array import speed_from_base
    cases = [
...
def test_scorch_height(ti):
    ...
    from components.behave_units_array import speed_to_base, speed_from_base
    ...
    # BehaveRunArray wrapper (with unit conversion)
    ...
    report(ti, "Scorch height via BehaveRunArray wrapper",
...
def test_crown_scorch_mortality(ti):
    ...
    # BehaveRunArray wrapper
    mort_w = _RUNNER.calculate_crown_scorch_mortality(
        np.array([30.0]), np.array([50.0]), np.array([0.5]),
        np.array([10.0]), np.array([2]),
    )
    report(ti, "Mortality via BehaveRunArray wrapper matches direct call",
...
def test_ignite(ti):
    ...
    # Douglas fir duff = type 5 in ignite_array.py; charge=2 (unknown)
...
def test_spot(ti):
    ...
    # BehaveRunArray wrappers return same values
    surf_r = _RUNNER.calculate_spotting_from_surface_fire(
        np.array([scalar_fl_ft]), np.array([5.0]), np.array([30.0])
    ).item()
    report(ti, "Spotting from surface fire via BehaveRunArray wrapper",
...
# ---------------------------------------------------------------------------
# 19. BehaveRunArray facade — all method wrappers accessible
# ---------------------------------------------------------------------------

def test_facade_completeness(ti):
    print("\n" + "=" * 70)
    print("Testing BehaveRunArray facade completeness")
    print("=" * 70)
    ...
    for m in required_methods:
        report(ti, f"BehaveRunArray.{m} exists",
...
def run_all_tests():
    print("\n" + "=" * 70)
    print("BEHAVE ARRAY MODULES — TEST SUITE")
    print("=" * 70)
